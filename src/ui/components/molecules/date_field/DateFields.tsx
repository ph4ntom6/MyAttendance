...
const DateFields: FC<Props> = ({
  ...
  const returnCallback = useCallback(
    ...
    [isOpenFromRequestView, setDateFields, previousMonthDate, currentMonthDate],
  );
    ...
    <>
      <View style={styles.flexContainer}>
        ...
        >
          ...
          <AppLabel
            ...
            text={formatDate(String(previousMonthDate), 'MMM DD, YY')}
          />
        </Pressable>
        <Pressable
          onPress={() => {
          ...
          style={[styles.dateContainer, { marginHorizontal: SPACE.md }]}
        >
          <Image
            source={require('assets/images/ic_calendar.png')}
            style={styles.dateIcon}
          ...
          <AppLabel
            style={styles.dateText}
            text={formatDate(String(currentMonthDate), 'MMM DD, YY')}
          />
        </Pressable>
        <Pressable
          onPress={() => {
      ...
      {showDatePicker && (
        <DateTimePickerModal
          ...
          maximumDate={
            currentDateRef.current === 'Right' && isOpenFromRequestView
              ? new Date(new Date().setFullYear(new Date().getFullYear() + 1))
              : new Date()
          }
          modalStyleIOS={{
            marginBottom:
              safeAreaInset.bottom > 0 ? safeAreaInset.bottom : SPACE.lg,
          }}
        />
      )}
    </>
