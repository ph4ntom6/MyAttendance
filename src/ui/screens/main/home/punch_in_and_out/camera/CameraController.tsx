        ...
        if (hasError || dataBody === undefined) {
            if (statusCode === 567) {
                ...
                navigation.replace('MissingPunchOut', {
                    response: (errorBody as any)
                        ?.data as MissingPunchOutResponseModel,
        ...
        if (
            Platform.OS === 'android'
                ? checkPerm[PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION] ===
                  RESULTS.GRANTED
                : checkPerm[PERMISSIONS.IOS.LOCATION_ALWAYS] ===
                      RESULTS.GRANTED ||
                  checkPerm[PERMISSIONS.IOS.LOCATION_WHEN_IN_USE] ===
                      RESULTS.GRANTED
        ) {
            getUserLocation()
        } else {
