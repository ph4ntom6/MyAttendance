...
const AttendanceController: FC<Props> = () => {
    ...
    const getAttendanceData = usePreventDoubleTap(
        async (clearData: boolean = false) => {
            ...
            const { hasError, dataBody, errorBody } =
                await getAttendanceDataRequest(requestModel.current)
            if (hasError && dataBody === undefined) {
                SimpleToast.show(
                    errorBody ?? STRINGS.common.some_thing_bad_happened
                )
            } else {
                const dataFromCreationDate = dataBody.data.records.filter(
                    (_item) => {
                        ...
                        let date = dayjs(
                            splittedDate.join(', '),
                            'MMM D, YYYY'
                        ).isSameOrAfter(dayjs(user?.created_at, 'MMM D, YYYY'))
    ...
    useLayoutEffect(() => {
        navigation.setOptions({
            ...
            headerTitle: () => (
                <HeaderTitle text={STRINGS.home.attendance.title} />
            ),
        })
    }, [navigation])
